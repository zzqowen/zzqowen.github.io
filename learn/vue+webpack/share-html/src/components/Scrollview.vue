<template>
  <div class="my-scroll" :class="[scrollState?'prohibit':'allow']" ref="myScroll" @scroll.passive="onScroll($event)"
    ...
    <div class="scroll-list" :style="{ transform: 'translate3d(0, ' + top + 'px, 0)'}">
      ...
      <div class="scroll-bottom">
        <div v-if="state==4" class="pullUpText">加载中</div>
        <div v-if="state==5" class="pullUpText">加载完成</div>
        <div v-if="state==7" class="no_more pullUpText flex-row-center-container">已经到底啦</div>
      </div>
    </div>
  </div>
</template>
      ...
      touchMove(e) { //触摸滑动事件
        ...
        if (this.scrollState && e.targetTouches[0].pageY > this.pageY) { //向上滑动
          ...
          if (this.myScroll.scrollTop == 0) {
            ...
            let ranget = diff / document.body.clientHeight * 60 //计算在屏幕上滑动了多少
            if (ranget > 10) {
              this.state = 2
            } else if (ranget < 5) {
              this.state = 6
